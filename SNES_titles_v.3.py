...
if response.status_code == 200:
    ...
    for row in rows:
        ...
        if len(columns) == 7:
            ...
            if 'Unreleased' not in na_status and 'Cancelled' not in na_status:
                ...
                #  THERE IS NO <a> element in TITLE
                    ...
                    platform = 'Super Nintendo Entertainment System'     #actually needed for NES
                ...
                if developer_elements:
                    ...
                    combined_text = columns[1].get_text(strip=True)
                    last_comma_index = max(combined_text.rfind(','), combined_text.rfind('(UK)'), combined_text.rfind('(JP)'), combined_text.rfind('(NA)'), combined_text.rfind('(EU)'))    #SNES SPECIFIC GAME CUTTHROAT ISLAND
                    if last_comma_index != -1:      #wowdom 
                        developer = combined_text[:last_comma_index]                # Remove the last comma and all text after it
                    else:
                        developer = columns[1].get_text(strip=True)


                # PUBLISHER ✍️     #FIX FOR ALL BELOW 👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇👇
                # sup_elements = soup.find_all('small')   #snes exlclusive SMALL
                sup_elements = columns[2].find_all('small')   #snes exclusive SMALL
                for sup in sup_elements:    # Before looping through sup_elements, remove any HTML tags inside each <sup> element
                    for tag in sup.find_all():
                        tag.extract()
                sup = columns[2].find('small', string=re.compile(r'NA', re.IGNORECASE))   # "Publisher(s)" column (index 2)
                if sup:
