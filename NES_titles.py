        ...
        if len(columns) == 7:
            ...
            if 'Unreleased' not in na_status:
                ...
                #  THERE IS NO <a> element in TITLE
                    ...
                    platform = 'Nintendo Entertainment System'     #actually needed for NES
                ...
                # DEVELOPER 👨‍💻 #NES SPECIFIC
                #OG
                # developer_elements = columns[1].find_all('a')
                # developer = ', '.join([developer.get_text(strip=True) for developer in developer_elements]) if developer_elements else columns[1].get_text(strip=True)
                #BETTERDOM
                developer_elements = columns[1].find_all(string=True)
                developer = ', '.join([developer.get_text(strip=True) for developer in developer_elements if developer.get_text(strip=True)])   #WOW?
